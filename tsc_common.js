...
function createDyGraph(data, titleText, offset = 0) {
  var grph = new Dygraph(document.getElementById("graph"), data, {
    labels: origLabels,
    strokeWidth: 2.0,
    ...
    axes: {
      y: { valueRange: [-50 + offset, 1 + offset] },
      x: {
        logscale: true,
...
function tscGraph(
  ...
  this.createDyGraph = function () {
    ...
    this.graph = new Dygraph(this.div, this.data, {
      labels: this.labels,
      strokeWidth: 2.0,
      colors: this.colors,
      title: this.title,
      ...
      axes: {
        y: {
          ...
          ticker: function (min, max, pixels) {
            return [
              { v: -180 },
              { label_v: -180, label: "-180&deg" },
              { v: -135 },
              { label_v: -135, label: "-135&deg" },
              { v: -90 },
              { label_v: -90, label: "-90&deg" },
              { v: -45 },
              { label_v: -45, label: "-45&deg" },
              { v: 0 },
              { label_v: 0, label: "0&deg" },
              { v: 45 },
              { label_v: 45, label: "45&deg" },
              { v: 90 },
              { label_v: 90, label: "90&deg" },
...
function doCalcBode(
  ...
  for (var j = 0; j < n; j++) {
    ...
    var magnitude =
      Math.sqrt(NUMERATOR_Re ** 2 + NUMERATOR_Im ** 2) / DENOMINATOR;
